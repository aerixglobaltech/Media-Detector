...
def test_connection():
    print("--- CAMERA CONNECTION TEST (UNENCODED) ---")
    ...
    # Construction WITHOUT encoding
    url = f"rtsp://{user}:{password}@{ip}:{port}{path}"
    
    print(f"\nConstructed RAW URL: rtsp://{user}:*******@{ip}:{port}{path}")
    print("Connecting...")
    ...
    if cap.isOpened():
        ...
        if ret:
            print(f"\nSUCCESS! Surprisingly the raw URL worked.")
        else:
            print("\nPARTIAL SUCCESS: Linked, but no video.")
    else:
        elapsed = time.time() - start_time
        print(f"\nFAILED: Could not open the stream (Time: {elapsed:.2f}s).")
        print("This is likely because the special character broke the URL format.")
